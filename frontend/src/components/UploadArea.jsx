...
const UploadArea = ({ label, type, file, preview, setFile, accept, multiple }) => {
  ...
  return (
    <div className="flex flex-col gap-2 flex-1">
      ...
      <div
        onClick={() => !hasFile ? inputRef.current?.click() : null}
            ...
            {type === 'image' && preview ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                ...
                <span className="text-xs text-gray-400 truncate max-w-[80%]">
                  {multiple ? `${file.length} items uploaded` : (file.name || 'Video uploaded')}
                </span>
              </div>
            )}
